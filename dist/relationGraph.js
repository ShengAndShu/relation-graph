...
/******/ 	// eslint-disable-next-line no-unused-vars
/******/ 	var hotCurrentHash = "1a823c4f0b5e39922193";
...
// CONCATENATED MODULE: ./assets/js/echarts-callsAnls.js
var require;
var require, define;
(function () {
    var mods = {};
...
const utils_rectDom = document.createElement('div');
utils_rectDom.id = "graphDraw";
utils_rectDom.style.position = 'absolute';
utils_rectDom.style.backgroundColor = 'rgba(210,210,210,0.4)';
utils_rectDom.style.cursor = 'move';

const utils = {
    force: null,
    rectDom: utils_rectDom,    // 框选用的dom
    delStore: [],        // 保存删除的节点
    filterStore: [],     // 保存过滤的连线
...
// CONCATENATED MODULE: ./src/event/eventHandler.js
const eventHandler = {
...
/* harmony default export */ var event_eventHandler = (eventHandler);
// CONCATENATED MODULE: ./src/event/chartEvent.js



const chartEvent = {
    FORCE_LAYOUT_END: function () {
        const nodes = utils_utils.force.series[0].nodes || [];
        ...
        utils_utils.force.fixed = true;
        event_eventHandler.onforcelayout();
        //setNodeCenter
    },
    // 单击
    CLICK: function (param) {
        const data = param.data;
        event_eventHandler.onclick(param);
        // 单击只针对节点有效，线条单击无效
        if (!data.source && !data.target) {
            const force = utils_utils.force,
                layer = force.zr.painter.getLayer(force.zlevel),
            ...
            if (data.hasLeaf &&
                ...
                if (data.showLeaf) {
                    //收缩节点
                    utils_utils.toggleLeaf(data.name, true);
                    utils_utils.refresh();
                } else {
                    //展开节点
                    const series = force.series[0],
                    ...
                    utils_utils.toggleLeaf(data.name, false);
                    if (utils_utils.filterFun) {
                        utils_utils.filter(utils_utils.filterFun);
                    } else {
                        utils_utils.refresh();
                    }
                    event_eventHandler.onshowleaf();
                    series.center = center;   // 恢复簇状布局的影响
                    series.gravity = gravity;   // 恢复簇状布局的影响
                }
                event_eventHandler.oncountchange(utils_utils.force._graph.nodes.map(d => d.id));
            } else {
                // 节点高亮
                data.isSelected = !data.isSelected;
                data.itemStyle = utils_utils.getNodeStyle(data);
                utils_utils.refresh();
            }
        }
    },
    ...
    CONTEXTMENU: function (param) {
        event_eventHandler.oncontextmenu(param);
    }
};
/* harmony default export */ var event_chartEvent = (chartEvent);
// CONCATENATED MODULE: ./src/event/drawEvent.js
...
const drawEvent = {
    mousedown: function(ev) {
        const dom = utils_utils.force.zr.painter._domRoot,
    ...
    mouseup: function() {
        ...
        utils_utils.setRoam(true, true, false);            // 恢复画布拖动和缩放
        utils_utils.refresh();
        ...
        utils_utils.isDrawing = false;
        utils_utils.force.zr.handler.isDrawing = false;   // 变换鼠标样式
        utils_utils.rectDom.style.height = '0';
    },
    dblclick: function () {
        const dom = utils_utils.force.zr.painter._domRoot;
        utils_utils.deleteRect();
        utils_utils.refresh();
        dom.removeEventListener('dblclick', drawEvent.dblclick);
    }
};

/* harmony default export */ var event_drawEvent = (drawEvent);
...
class src_RelationGraph {
    constructor(el) {
        this.utils = utils_utils;
        this.forceChart = echarts.init(el);
        // 绑定chartEvent
        for (let ev in event_chartEvent) {
            this.forceChart.un(config.EVENT[ev], event_chartEvent[ev]);
            this.forceChart.on(config.EVENT[ev], event_chartEvent[ev]);
        }

        const _domRoot = this.forceChart._zr.painter._domRoot;
        _domRoot.oncontextmenu = () => false;
        _domRoot.appendChild(utils_utils.rectDom);  // 添加矩形dom，用来框选
    }
    ...
    setOption(option) {
        ...
        utils_utils.option = Object.assign(utils_utils.option, option);
        utils_utils.delStore = [];
        // 若传节点数据，则先清空，再重新setOption
        if (!force || leafData.length || data.length || rootData.length) {
            const size = utils_utils.getLayoutSize(rootData.length + data.length);
            const chartOption = utils_utils.getChartOption(size, [], []);
            // 清空上次的数据
            utils_utils.delStore = [];
            this.forceChart.clear();
            // setOption
            this.forceChart.setOption(chartOption);
            utils_utils.force = this.forceChart.chart.force;
            // 添加数据
            this.addRootNodes(rootData, false);
            this.addNodesLinks(data, false);
            this.addNodesLinks(leafData, true);
            const series = utils_utils.force.series[0];
            series.nodes = utils_utils.formatNodes(series.nodes);
            series.links = utils_utils.formatLinks(series.links);
            utils_utils.force.refresh();
    ...
    addRootNodes(rootData, onlyRootData) {
        ...
        for (let i = 0, len = rootData.length; i < len; i++) {
            ...
            node.itemStyle = utils_utils.getNodeStyle(node);
            node.symbol = utils_utils.getNodeImage(node);
            node.z = utils_utils.getNodeZ(node);
            node.isCritical = utils_utils.getNodeCritical(node);
            nodes.push(node);
            nameList.push(node.name);
        }
        needDeeplyDel.length && this.deeplyDelete(needDeeplyDel);
        if (onlyRootData) {
            utils_utils.option.rootData = utils_utils.option.rootData.concat(rootData);
            force.fixed = false;
            force.refresh();
            event_eventHandler.oncountchange(utils_utils.force._graph.nodes.map(d => d.id));
        }
     ...
     */
    on(eventType, cb) {
        event_eventHandler['on' + eventType] = cb;
    }
    ...
    deleteNodes(nameList) {
        ...
        utils_utils.deleteRect();
        utils_utils.refresh();
        const singleDel = utils_utils.delSingleNode();     // 节点删除后画布可能会出现孤立节点，一并删除
        delList = [...delList, ...singleDel];
        if (delList.length > 0) {
            if (utils_utils.delStore.length === utils_utils.option.deleteCacheLength) {
                this.deeplyDelete(utils_utils.delStore.shift());
            }
            utils_utils.delStore.push(delList);
            utils_utils.refresh();
            event_eventHandler.oncountchange(utils_utils.force._graph.nodes.map(d => d.id));
            event_eventHandler.ondeletenodes(delList);
        }
        return delList;
    }
     ...
     */
    revertLastDel() {
        if (utils_utils.delStore.length === 0) {
            return;
        }
        const lastDel = utils_utils.delStore[utils_utils.delStore.length - 1];
        this.revertDel(lastDel);
        utils_utils.delStore.pop();
    }
     ...
     */
    revertDel(nameList) {
        utils_utils.revertDel(nameList);
        utils_utils.refresh();
        event_eventHandler.oncountchange(utils_utils.force._graph.nodes.map(d => d.id));
        event_eventHandler.onrevert(nameList);
    }
    ...
    deeplyDelete(names) {
        const force = utils_utils.force;
        if (!force) return;
        names = names || utils_utils.delStore[0] || [];
    ...
    drawRect() {
        ...
        if (utils_utils.isDrawing || force.dragBox) {
            this.deleteRect();
            utils_utils.refresh();
            return;
        }
        utils_utils.isDrawing = true;
        utils_utils.force.zr.handler.isDrawing = true;   // 变换鼠标样式
        utils_utils.setRoam(false, false, true);   // 禁用画布拖动和缩放
        dom.addEventListener('mousedown', event_drawEvent.mousedown);
    }
    ...
    deleteRect() {
        ...
        if (!force) return;
        const dom = force.zr.painter._domRoot;
        dom.removeEventListener('mousedown', event_drawEvent.mousedown);
        dom.removeEventListener('mousemove', event_drawEvent.mousemove);
        dom.removeEventListener('mouseup', event_drawEvent.mouseup);
        utils_utils.deleteRect();
    }
    ...
    markNodes() {
        ...
        for (let i = 0, l = nodes.length; i < l; i++) {
            ...
            if (node.isSelected) {
                ...
                node.itemStyle = utils_utils.getNodeStyle(node);  // 取消高亮
                node.symbol = utils_utils.getNodeImage(node);     // 更换图片
            }
        }
        utils_utils.deleteRect();
        utils_utils.refresh();
    ...
    searchNode(name) {
        ...
        for (let i = 0, l = nodes.length; i < l; i++) {
            ...
            if (node.isSearched) {      // 上一个被搜索的节点样式还原
                ...
                node.z = utils_utils.getNodeZ(node);
                node.itemStyle = utils_utils.getNodeStyle(node);   // 取消高亮
                node.symbolSize = utils_utils.getNodeSize(node);   // 图片大小恢复
                if (name === '') {
                    break;
                }
            }
            if (node.name === name) {   // 当前被选中的节点变更样式
                ...
                node.z = utils_utils.getNodeZ(node);
                node.itemStyle = utils_utils.getNodeStyle(node);   // 文字高亮
                node.symbolSize = utils_utils.getNodeSize(node);   // 图片放大
                utils_utils.setNodeCenter(name);
            }
        }
        utils_utils.refresh();
    }
    ...
    layout() {
        ...
        series.gravity = .8;
        series.size = utils_utils.getLayoutSize(force._layout.graph.nodes.length);
        utils_utils.deleteRect();
        utils_utils.refresh();
    }
    ...
    clear() {
        this.forceChart.clear();
        this.forceChart.setOption(utils_utils.getChartOption('100%', [], []));
        utils_utils.force = this.forceChart.chart.force;
        utils_utils.delStore = [];
        utils_utils.isDrawing = false;
        event_eventHandler.oncountchange([]);
    }
     ...
     */
    toogleCritical() {
        utils_utils.toggleCritical();
    }
}

const relationGraph = {
    init: element => new src_RelationGraph(element)
};
window.relationGraph = relationGraph;
/* harmony default export */ var src = __webpack_exports__["default"] = (relationGraph);
